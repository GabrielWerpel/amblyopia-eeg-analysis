...
% Use the best feature combination identified previously
% For this example, let's assume the best features are stored in 'best_features'
% If not defined, please run the classification code to obtain 'best_features'

% For the sake of this example, let's define best_features (replace with your actual best features)
best_features = [1, 4]; % Example feature indices
best_feature1_label = feature_labels{best_features(1)};
best_feature2_label = feature_labels{best_features(2)};

% Extract data for the best features
data = all_features(:, best_features);

% Original class labels
true_labels = class_division;

% Original model performance (from previous code)
% We'll compute it here using the same procedure

% Initialize arrays to store predictions and true labels across all folds
y_true_all = [];
y_pred_all = [];

% Set up cross-validation partition
cv = cvpartition(length(class_division), 'LeaveOut'); % LOOCV independent of labels
...
for i = 1:cv.NumTestSets
    ...
    y_train = true_labels(trainIdx);
    X_test = data(testIdx, :);
    y_test = true_labels(testIdx);
...
original_accuracy = 100 * (tp + tn) / (tp + tn + fp + fn);
original_mcc = ((tp * tn) - (fp * fn)) / sqrt((tp + fp)*(tp + fn)*(tn + fp)*(tn + fn) + eps);
original_sensitivity = 100 * tp / (tp + fn + eps);
original_specificity = 100 * tn / (tn + fp + eps);

% Display original performance
disp('Original Model Performance:');
...
% Number of permutations (times we flip labels)
num_permutations = 10; % You can adjust this number as needed

% Number of labels to flip in each permutation
num_labels_to_flip = 2; % Adjust as needed (e.g., 1 or 2)

% Store accuracies and MCCs from label flipping tests
flipped_accuracies = zeros(num_permutations, 1);
flipped_mccs = zeros(num_permutations, 1);

for perm = 1:num_permutations
    % Copy the true labels
    flipped_labels = true_labels;
    
    % Randomly select indices to flip
    indices_to_flip = randperm(length(flipped_labels), num_labels_to_flip);
    
    % Flip the labels
    for idx = indices_to_flip
        if flipped_labels(idx) == 1
    ...
    % Cross-validation partition (same as before)
    cv_flipped = cv; % Use the same partitioning for consistency
    
    % Loop over each fold (Leave-One-Out Cross-Validation)
    for i = 1:cv_flipped.NumTestSets
        trainIdx = cv_flipped.training(i);
        testIdx = cv_flipped.test(i);
        
        % Training and test data
        X_train = data(trainIdx, :);
        y_train = flipped_labels(trainIdx);
        X_test = data(testIdx, :);
        y_test = flipped_labels(testIdx);
        
        % Train Random Forest classifier
        numTrees = 10; % Same number as used previously
        rfModel = TreeBagger(numTrees, X_train, y_train, 'Method', 'classification', 'OOBPrediction', 'off');
        ...
        % Store true labels and predictions
        y_true_all_flipped = [y_true_all_flipped; y_test];
        y_pred_all_flipped = [y_pred_all_flipped; y_pred];
    end
    
    % Compute confusion matrix components
    tp = sum((y_true_all_flipped == 1) & (y_pred_all_flipped == 1));
    tn = sum((y_true_all_flipped == 0) & (y_pred_all_flipped == 0));
    fp = sum((y_true_all_flipped == 0) & (y_pred_all_flipped == 1));
    fn = sum((y_true_all_flipped == 1) & (y_pred_all_flipped == 0));
    
    % Calculate performance metrics
    accuracy_flipped = 100 * (tp + tn) / (tp + tn + fp + fn);
    mcc_flipped = ((tp * tn) - (fp * fn)) / sqrt((tp + fp)*(tp + fn)*(tn + fp)*(tn + fn) + eps);
    
    % Store metrics
    flipped_accuracies(perm) = accuracy_flipped;
    flipped_mccs(perm) = mcc_flipped;
    
    % Display results for this permutation
    fprintf('\nPermutation %d:\n', perm);
    fprintf('Flipped Labels at Indices: %s\n', mat2str(indices_to_flip));
    fprintf('Accuracy after flipping: %.2f%%\n', accuracy_flipped);
    fprintf('MCC after flipping: %.4f\n', mcc_flipped);
end

% Analyze the impact of label flipping
mean_flipped_accuracy = mean(flipped_accuracies);
mean_flipped_mcc = mean(flipped_mccs);

% Display summary
fprintf('\nSummary of Label Flipping Test:\n');
fprintf('Original Accuracy: %.2f%%\n', original_accuracy);
fprintf('Mean Accuracy after Flipping Labels: %.2f%%\n', mean_flipped_accuracy);
fprintf('Original MCC: %.4f\n', original_mcc);
fprintf('Mean MCC after Flipping Labels: %.4f\n', mean_flipped_mcc);

% Optionally, plot the accuracies
figure;
bar(1:num_permutations, flipped_accuracies);
hold on;
plot(xlim, [original_accuracy, original_accuracy], 'r--', 'LineWidth', 2);
xlabel('Permutation Number');
ylabel('Accuracy (%)');
title('Accuracy after Flipping Labels');
legend('Flipped Accuracies', 'Original Accuracy');
hold off;
