...
for ix_feat = 1:size(IX_FEATURES_, 1)
    ...
    mccs = zeros(cv.NumTestSets, 1);
    sensitivities = zeros(cv.NumTestSets, 1);
    specificities = zeros(cv.NumTestSets, 1);
    ...
    all_results = [all_results; [avg_accuracy, avg_mcc, avg_sensitivity, avg_specificity, IX_FEATURES]];
end
...
results_table = array2table(all_results, 'VariableNames', {'Accuracy', 'MCC', 'Sensitivity', 'Specificity', 'Feature1', 'Feature2'});
disp('Classification Results using SVM:');
disp(results_table);
...
disp('Best Feature Combination using SVM:');
...
% (Shuffle test code would be similar to before, adapted for SVM)