...
% Now, we will consider all combinations of 2 features from 12.
...
for ix_feat = 1:size(IX_FEATURES_, 1)
    ...
    % Extract the training data for the selected features.
    training = all_features(:, IX_FEATURES);
    
    % Class labels remain the same.
    group = class_division;
    
    % Specify the type of classifier (Linear Discriminant Analysis).
    CLASSIF_TYPE = 'linear'; % You might also experiment with 'diaglinear'
    
    % Train the classifier and obtain coefficients.
    % Here, 'ones(1, size(training, 2))' is a dummy sample to extract the coefficients.
    [CLASS, ERR, POSTERIOR, ~, COEF] = classify(ones(1, size(training, 2)), training, group, CLASSIF_TYPE);
    
    % Extract the coefficients for the linear decision boundary.
    K = COEF(1, 2).const;     % Constant term
    L = COEF(1, 2).linear;    % Linear coefficients
    ...
    for ii = 1:size(training, 1)
        % Index of the sample to hold out.
        ix_holdout = ii;
        
        % Indices of the samples to include in training.
        ix_holdin = setdiff(1:size(training, 1), ix_holdout);
        
        % Training data and labels excluding the holdout sample.
        training_ = training(ix_holdin, :);
        group_ = group(ix_holdin);
        
        % Test data (the holdout sample).
        test_sample = training(ix_holdout, :);
        
        % Train the classifier on the training data.
        [CLASS, ~, ~, ~, ~] = classify(test_sample, training_, group_, CLASSIF_TYPE);
        
        % Update the confusion matrix based on the true label and prediction.
        true_label = group(ix_holdout);
        predicted_label = CLASS;
        
        if true_label == 0  % Control subject
            if predicted_label == 0, cm_loocv(1, 1) = cm_loocv(1, 1) + 1; end  % True Negative
            if predicted_label == 1, cm_loocv(1, 2) = cm_loocv(1, 2) + 1; end  % False Positive
        else  % Amblyopia patient
            if predicted_label == 0, cm_loocv(2, 1) = cm_loocv(2, 1) + 1; end  % False Negative
            if predicted_label == 1, cm_loocv(2, 2) = cm_loocv(2, 2) + 1; end  % True Positive
        end
    end
    ...
    [mcc, sens, spec] = MCC(cm_loocv);
    accuracy = 100 * sum(diag(cm_loocv)) / sum(cm_loocv(:));
    
    % Store the results: [Accuracy, MCC, Sensitivity, Specificity, Feature Indices]
    all_results = [all_results; [accuracy, mcc, sens*100, spec*100, IX_FEATURES]];
...
disp('Accuracy (%)    MCC       Sensitivity (%)   Specificity (%)   Feature 1    Feature 2');
disp(all_results);
...
% Extract the training data for the best features.
training = all_features(:, best_features);

% Shuffle Test to assess statistical significance.
N_SHUFFLE = 2000;      % Number of shuffle iterations
FLAG_SHUFFLE = true;   % Flag to perform the shuffle test

if FLAG_SHUFFLE
    % Function to shuffle the group labels.
    fnShuffle = @(x) x(randperm(length(x)));
    
    % Initialize an array to store shuffled performance metrics.
    perf_shuf = [];
    disp('Performing shuffle test...');
    
    % Loop over shuffle iterations.
    for iishuf = 1:N_SHUFFLE
        
        % Randomize the group labels.
        group_shuf = fnShuffle(group);
        
        % Initialize a confusion matrix for LOOCV.
        cm_loocv = zeros(2, 2);
        
        % Perform LOOCV with shuffled labels.
        for ii = 1:size(training, 1)
            ix_holdout = ii;
            ix_holdin = setdiff(1:size(training, 1), ix_holdout);
            
            training_ = training(ix_holdin, :);
            group_ = group_shuf(ix_holdin);
            test_sample = training(ix_holdout, :);
            
            [CLASS, ~, ~, ~, ~] = classify(test_sample, training_, group_, CLASSIF_TYPE);
            
            true_label = group_shuf(ix_holdout);
            predicted_label = CLASS;
            
            if true_label == 0
                if predicted_label == 0, cm_loocv(1, 1) = cm_loocv(1, 1) + 1; end
                if predicted_label == 1, cm_loocv(1, 2) = cm_loocv(1, 2) + 1; end
            else
                if predicted_label == 0, cm_loocv(2, 1) = cm_loocv(2, 1) + 1; end
                if predicted_label == 1, cm_loocv(2, 2) = cm_loocv(2, 2) + 1; end
            end
        end
        
        % Calculate performance metrics for the shuffled data.
        [mcc_shuf, ~, ~] = MCC(cm_loocv);
        accuracy_shuf = 100 * sum(diag(cm_loocv)) / sum(cm_loocv(:));
        
        % Store the shuffled performance metrics.
        perf_shuf = [perf_shuf; [accuracy_shuf, mcc_shuf]];
        
    end % End of shuffle loop
    
    % Calculate and display statistics from the shuffled performance.
    mean_perf = mean(perf_shuf, 1);
    acc_percentiles = prctile(perf_shuf(:, 1), [2.5, 97.5]);
    mcc_percentiles = prctile(perf_shuf(:, 2), [2.5, 97.5]);
    
    disp('Shuffle Test Results:');
    fprintf('Mean Accuracy (%%): %.2f\n', mean_perf(1));
    fprintf('Accuracy 95%% CI: [%.2f, %.2f]\n', acc_percentiles(1), acc_percentiles(2));
    fprintf('Mean MCC: %.4f\n', mean_perf(2));
    fprintf('MCC 95%% CI: [%.4f, %.4f]\n', mcc_percentiles(1), mcc_percentiles(2));
end % End of shuffle test

% Function to calculate Matthews Correlation Coefficient (MCC),
% Sensitivity, and Specificity from a confusion matrix.
function [mcc, sens, spec] = MCC(cm)
