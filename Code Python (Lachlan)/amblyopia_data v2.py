...
from machine_learning_common.features.features_common import compute_power_frequency_features
...
def parse_data(cohort_map: dict) -> mne.BaseEpochs:
    ...
    for filename in os.listdir(BASE_DATA_PATH):
        ...
        dominant_eye = cohort_map.get(participant_id, None)
        ...
        for condition_idx in range(raw_data.shape[1]):
            ...
            metadata = pd.concat([metadata, alpha_ref_df.reset_index(drop=True)], axis=1)
