function segmentedData = segmentEEGDataNew1s(results)
    ...
    for i = 1:numel(fields)
        ...
        if strcmp(cohortRow.LC, 'DEC')
            ...
            DEC = LC(samples_to_remove + 1:end);
            NDEC = RC(samples_to_remove + 1:end);
        end
        if strcmp(cohortRow.RC, 'DEC')
            % Apply 'DEC' condition to 'RC'
            DEC = RC(samples_to_remove + 1:end);
            NDEC = LC(samples_to_remove + 1:end);
        end
