function plotReactivityScatterPlots(reactivityResults, segmentedData, columnName, titleName)
    ...
    newLabels = strrep(categories(dataA.Section), '_', '-');
    dataA.Section = renamecats(dataA.Section, newLabels);
    dataC.Section = renamecats(dataC.Section, newLabels);
    ...
    % Directly use LinesDifference for color intensity (only for Amblyopia)
    linesDiffA = dataA.LinesDifference;
    ...
    % Increase the contrast for color intensity mapping
    normLinesDiffA = 1 - normLinesDiffA;
    ...
    markerSize = 100;  % Increase the marker size
    for i = 1:numel(dataA.Key)
        key = dataA.Key{i};
        marker = markerMap(key);
        scatter(xA(i), dataA.Reactivity(i), markerSize, [0, 0, 1] .* (0.5 + 0.5 * normLinesDiffA(i)), marker, 'filled');
    end
    ...
    for i = 1:numel(dataC.Key)
        key = dataC.Key{i};
        marker = markerMap(key);
        scatter(xC(i), dataC.Reactivity(i), markerSize, 'r', marker, 'filled');
    end
    ...
    for i = 1:numel(uniqueKeys)
        ...
        if contains(key, 'A')
            ...
            intensity = linesDiffA(strcmp(dataA.Key, key));  % Get the intensity level directly
            legendLabels{i} = sprintf('%s (%.0f)   ', key, intensity);  % Append intensity to the label
        else
            color = 'r';  % Red for Control
            legendLabels{i} = key;  % No intensity value for Control
        end
        legendHandles(i) = plot(nan, nan, marker, 'MarkerFaceColor', color, 'MarkerEdgeColor', color);
    end

    % Enhance the plot
    ylabel(columnName);
    xlabel('Section');
    xticks(1:length(newLabels));
